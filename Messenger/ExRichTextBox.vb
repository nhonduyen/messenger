...
#Region "My Privates"
        ...
        Private rtfColor_ As HybridDictionary
...
#Region "Constructors"
        ...
        Public Sub New()
            ...
            rtfColor_ = New HybridDictionary()
            rtfColor_.Add(RtfColor.Aqua, RtfColorDef.Aqua)
            rtfColor_.Add(RtfColor.Black, RtfColorDef.Black)
            rtfColor_.Add(RtfColor.Blue, RtfColorDef.Blue)
            rtfColor_.Add(RtfColor.Fuchsia, RtfColorDef.Fuchsia)
            rtfColor_.Add(RtfColor.Gray, RtfColorDef.Gray)
            rtfColor_.Add(RtfColor.Green, RtfColorDef.Green)
            rtfColor_.Add(RtfColor.Lime, RtfColorDef.Lime)
            rtfColor_.Add(RtfColor.Maroon, RtfColorDef.Maroon)
            rtfColor_.Add(RtfColor.Navy, RtfColorDef.Navy)
            rtfColor_.Add(RtfColor.Olive, RtfColorDef.Olive)
            rtfColor_.Add(RtfColor.Purple, RtfColorDef.Purple)
            rtfColor_.Add(RtfColor.Red, RtfColorDef.Red)
            rtfColor_.Add(RtfColor.Silver, RtfColorDef.Silver)
            rtfColor_.Add(RtfColor.Teal, RtfColorDef.Teal)
            rtfColor_.Add(RtfColor.White, RtfColorDef.White)
            rtfColor_.Add(RtfColor.Yellow, RtfColorDef.Yellow)
...
#Region "Insert Plain Text"
        ...
        Private Function GetDocumentArea(ByVal _text As String, ByVal _font As Font) As String
            ...
            ' Font.SizeInPoints
            _doc.Append("\fs")
            _doc.Append(CInt(Math.Truncate(Math.Round((2 * _font.SizeInPoints)))))
...
#Region "Insert Image"
        ...
        Private Function GetImagePrefix(ByVal _image As Image) As String
            ...
            Dim picw As Integer = CInt(Math.Truncate(Math.Round((_image.Width / xDpi) * HMM_PER_INCH)))

            ' Calculate the current height of the image in (0.01)mm
            Dim pich As Integer = CInt(Math.Truncate(Math.Round((_image.Height / yDpi) * HMM_PER_INCH)))

            ' Calculate the target width of the image in twips
            Dim picwgoal As Integer = CInt(Math.Truncate(Math.Round((_image.Width / xDpi) * TWIPS_PER_INCH)))

            ' Calculate the target height of the image in twips
            Dim pichgoal As Integer = CInt(Math.Truncate(Math.Round((_image.Height / yDpi) * TWIPS_PER_INCH)))
        ...
        Private Function GetRtfImage(ByVal _image As Image) As String
            ...
            Try
                ...
                Using _graphics As Graphics = Graphics.FromImage(_metaFile)
...
#Region "RTF Helpers"
        ...
        Private Function GetColorTable(ByVal _textColor As RtfColor, ByVal _backColor As RtfColor) As String
            ...
            _colorTable.Append(rtfColor_(_textColor))
            _colorTable.Append(";")

            ' Append the highlight color
            _colorTable.Append(rtfColor_(_backColor))
            _colorTable.Append(";}\n")
