...
Public Class frmGroupChat
    ...
    Public dGroupUsers As New Dictionary(Of String, String)
    ...
    Private Sub frmChat_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        bRoom = False
        For Each pair In dGroupUsers
            cUdpServer.ReplyInvite(pair.Key, MES_EXIT_GROUP, Me.Text)
        Next
    End Sub

    Public Sub frmChatClose()
        bRoom = False
        For Each pair In dGroupUsers
            cUdpServer.ReplyInvite(pair.Key, MES_EXIT_GROUP, Me.Text)
        Next
    ...
    Public Sub frmChat_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        End If
        'Load icon
        tPatner.ImageList = frmMain.images
        tPatner.ImageIndex = 1
        tPatner.SelectedImageIndex = 1

        UpdatePatner()

    End Sub

    Public Sub UpdatePatner()
        tPatner.Nodes.Clear()
        For Each pair In dictContacts
            If pair.Key.ToString <> GetIpAdd() Then
                ...
                tPatner.Nodes.AddRange(New TreeNode() {node})
            End If
        Next
        tPatner.ExpandAll()
    End Sub

    Private Sub tPatner_NodeMouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TreeNodeMouseClickEventArgs) Handles tPatner.NodeMouseClick
        If e.Button = Windows.Forms.MouseButtons.Right Then
            tPatner.SelectedNode = e.Node
            If (Not tPatner.SelectedNode Is Nothing) Then
                contextPater.Show(Me.tPatner, e.Location)
            End If
        End If
    End Sub

    Private Sub TextBox1_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tbxSearch.TextChanged
    ...
    Private Sub btnSend_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSend.Click
        Try
            ...
            For Each pair In dGroupUsers
                cUdpServer.ChatGroup(pair.Key, MES_MESSENGE_GROUP, Me.Text.Trim)
            Next
            If bSoundOn = True Then
    ...
    Private Sub ToolStripButtonBuzz_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButtonBuzz.Click
        ...
        For Each pair In dGroupUsers
            cUdpServer.ChatGroup(pair.Key, MES_GROUP_BUZZ, Me.Text.Trim)
        Next
        If bSoundOn = True Then
            My.Computer.Audio.Play(My.Resources.buzz, AudioPlayMode.Background)
        End If
    ...
    Private Sub btnInvite_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnInvite.Click
        Dim czGroupUserIp As String = ""
        For Each pair In dGroupUsers
            czGroupUserIp = pair.Value + "<" + pair.Key + ">  |  "
        Next

        If czGroupUserIp <> "" Then
            czGroupUserIp.Remove(czGroupUserIp.LastIndexOf("|"))
        End If
        frmInvite.czGroupUserIp = czGroupUserIp
        frmInvite.Show()
    End Sub

    Private Sub AddToRoomToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AddToRoomToolStripMenuItem.Click
        Try
            Dim temp() As String = tPatner.SelectedNode.ToolTipText.ToString.Split(":")
            Dim ip_temp As String = temp(temp.Count - 1).ToString.Trim
            If dGroupUsers.ContainsKey(ip_temp) Then
                MessageBox.Show("This ip is containt on your group")
            Else
                Dim czGroupUserIp As String = ""
                For Each pair In dGroupUsers
                    czGroupUserIp = pair.Value + "<" + pair.Key + ">  |  "
                ...
                cUdpServer.SendInvite(ip_temp, MES_INVITE, czGroupUserIp)
    ...
    Public Sub UpdateGroupUser(ByVal ip As String)
        If dGroupUsers.ContainsKey(ip) Then
            dGroupUsers.Remove(ip)
            rtbGroupUsers.Text = ""
            For Each pair In dGroupUsers
                rtbGroupUsers.Text = rtbGroupUsers.Text + pair.Value + "<" + pair.Key + ">   |   "
            Next
        End If
    End Sub

    Public Sub UpdateGroupUser(ByVal ip As String, ByVal nickname As String, ByVal czBuff As String)
        If czBuff = "R_" + GetIpAdd() Then
            dGroupUsers.Add(ip, nickname)
            rtbGroupUsers.Text = ""
            For Each pair In dGroupUsers
                rtbGroupUsers.Text = rtbGroupUsers.Text + pair.Value + "<" + pair.Key + ">   |   "
            Next
        Else
            MessageBox.Show(nickname + " decline join your group chat")
        End If

    End Sub

    Public Sub UpdateGroupUser()
        rtbGroupUsers.Text = ""
        For Each pair In dGroupUsers
            rtbGroupUsers.Text = rtbGroupUsers.Text + pair.Value + "<" + pair.Key + ">   |   "
        Next
    End Sub
    Private Sub RemoveToolStripMenuItem_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RemoveToolStripMenuItem.Click
        Dim temp() As String = tPatner.SelectedNode.ToolTipText.ToString.Split(":")
        Dim ip_temp As String = temp(temp.Count - 1).ToString.Trim
        If dGroupUsers.ContainsKey(ip_temp) Then
            'cUdpServer.ChatGroup("255.255.255.255", MES_REMOVE_GROUP, Me.Text + "|" + tPatner.SelectedNode.ToolTipText.ToString.Split(":")(1).Trim)
            dGroupUsers.Remove(ip_temp)
            For Each pair In dGroupUsers
                cUdpServer.ChatGroup(pair.Key, MES_REMOVE_GROUP, Me.Text + "|" + ip_temp)
            Next
            UpdateGroupUser()
        Else
            MessageBox.Show("This ip is not containt on your group")
        End If
    End Sub
